...
This backend support session-based authentication which relies on HTTP-only cookies for session management, as well as jwt access token. If a frontend relies on session-based authentication, need to consider CORS issue as described below.

For frontend applications running on a different domain or port (e.g., `localhost:3001` for frontend, `localhost:3000` for backend), you must configure your HTTP client (like Axios or Fetch) to include credentials with every request.  It's a cross-origin request. By default, browsers do not send cookies in such scenarios.
